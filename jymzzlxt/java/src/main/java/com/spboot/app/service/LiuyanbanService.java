...
public class LiuyanbanService {
     ...
     *  根据id 获取一行数据
     */
    public R<Liuyanban> findById(Integer id) {
     ...
     *  根据Wrapper 对象进行数据筛选
     */
    public R<List<Liuyanban>> selectAll(Wrapper<Liuyanban> query) {
    ...
    /**
     *  根据map 条件筛选liuyanren字段等于session.username的数据并分页
     *
     */
    public R selectPagesLiuyanren(Map<String, Object> map) {
        // 获取前端给到列表基础参数
        SelectPage selectPage = new SelectPage(map, 10, "id", "DESC");
        // 将map参数转换为mybatis-plus的QueryWrapper类
        QueryWrapper<Liuyanban> wrapper = mapToWrapper(map);
        // 设置为当前角色
        wrapper.eq("liuyanren", SessionFactory.getUsername());
        wrapper.orderBy(true, selectPage.isAsc(), selectPage.getOrderby());
        Page page = new Page(selectPage.getPage(), selectPage.getPagesize());
        return selectPages(wrapper, page);
    }
    ...
    public R selectPages(QueryWrapper<Liuyanban> wrapper, IPage page) {
     ...
     * @return 是否处理成功
     */
    public R insert(Liuyanban entityData, Map post) {
        // 判断是否有填写咨询人姓名。
        if (StringUtil.isNullOrEmpty(post.get("zixunrenxingming"))) {
            return R.error("请填写咨询人姓名");
        }
        // 判断是否有填写联系电话。
        ...
        // 判断是否有填写留言内容。
        if (StringUtil.isNullOrEmpty(post.get("liuyanneirong"))) {
     ...
     * @return 是否处理成功
     */
    public R<Object> update(Liuyanban entityData, Map post) {
        // 判断是否有填写回复内容。
        if (StringUtil.isNullOrEmpty(post.get("huifuneirong"))) {
            return R.error("请填写回复内容");
        }
