...
public class UserService {
    ...
    public R<Object> login(String username, String pwd, String cx) {
        ...
        // 判断是否为"医生"角色登录时则使用这个进行登录
        if (cx.equals("医生")) {
            // 通过YishengService.login 方法进行登录

            Yisheng user = yishengService.login(username, pwd);
            // user 为null 则提示用户账号或密码错误
            if (user == null) {
                return R.error("账号或密码错误");
            }
            ...
            // 设置table 变量
            // 将bean 实体类转化为session
            session = yishengService.toSession(user);
        }
    ...
    public R<Object> editPassword(PasswordPojo passwordPojo) {
        ...
        if (roles.equals("医生")) {
            return yishengService.editPassword(SessionFactory.getId(), passwordPojo.getOldPassword(), passwordPojo.getNewPassword());
        }
