...
public class ShitiService {
     ...
     *  根据id 获取一行数据
     */
    public R<Shiti> findById(Integer id) {
     ...
     *  根据Wrapper 对象进行数据筛选
     */
    public R<List<Shiti>> selectAll(Wrapper<Shiti> query) {
    ...
    public R<List<Shiti>> selectAll() {
        QueryWrapper<Shiti> wrapper = Wrappers.query();
    ...
    public R selectAll(Map<String, Object> map) {
        ...
        QueryWrapper<Shiti> wrapper = mapToWrapper(map);
        // 设置排序
     ...
     */
    public QueryWrapper<Shiti> mapToWrapper(Map<String, Object> map) {
        // 创建 QueryWrapper 对象
        QueryWrapper<Shiti> wrapper = Wrappers.query();
    ...
    public R selectPages(QueryWrapper<Shiti> wrapper, IPage page) {
     ...
     * @return 是否处理成功
     */
    public R insert(Shiti entityData, Map post) {
        // 判断是否有填写试题题目。
        if (StringUtil.isNullOrEmpty(post.get("shititimu"))) {
            return R.error("请填写试题题目");
        }
        // 判断是否有填写类型。
        if (StringUtil.isNullOrEmpty(post.get("leixing"))) {
     ...
     * @return 是否处理成功
     */
    public R<Object> update(Shiti entityData, Map post) {
        // 判断是否有填写试题题目。
        if (StringUtil.isNullOrEmpty(post.get("shititimu"))) {
            return R.error("请填写试题题目");
        }
