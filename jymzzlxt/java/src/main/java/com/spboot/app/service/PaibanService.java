...
import com.spboot.app.mapper.PaibanMapper;
...
public class PaibanService {
    ...
    // 获取数据库操作类mapper
    @Resource
    private PaibanMapper mapper;

    /**
     *  根据id 获取一行数据
     */
    public R<Paiban> findById(Integer id) {
     ...
     *  根据Wrapper 对象进行数据筛选
     */
    public R<List<Paiban>> selectAll(Wrapper<Paiban> query) {
     ...
     *  直接筛选所有数据
     */
    public R<List<Paiban>> selectAll() {
        QueryWrapper<Paiban> wrapper = Wrappers.query();
    ...
    public R selectAll(Map<String, Object> map) {
        ...
        QueryWrapper<Paiban> wrapper = mapToWrapper(map);
        // 设置排序
     ...
     */
    public QueryWrapper<Paiban> mapToWrapper(Map<String, Object> map) {
        // 创建 QueryWrapper 对象
        QueryWrapper<Paiban> wrapper = Wrappers.query();
        ...
        // 判断URL 参数yishengid是否大于0
        if (!StringUtil.isNullOrEmpty(map.get("yishengid")) && Convert.toInt(map.get("yishengid")) > 0) {
            // 大于0 则写入条件
            wrapper.eq("yishengid", map.get("yishengid"));
        }

        if (!StringUtil.isNullOrEmpty(map.get("xingming"))) {
            wrapper.like("xingming", map.get("xingming"));
        }
        if (!StringUtil.isNullOrEmpty(map.get("shangbanshijian"))) {
            wrapper.like("shangbanshijian", map.get("shangbanshijian"));
        }
        if (!StringUtil.isNullOrEmpty(map.get("xiabanshijian"))) {
            wrapper.like("xiabanshijian", map.get("xiabanshijian"));
        }
    ...
    public R selectPages(QueryWrapper<Paiban> wrapper, IPage page) {
     ...
     * @return 是否处理成功
     */
    public R insert(Paiban entityData, Map post) {
        // 判断是否有填写上班时间。
        if (StringUtil.isNullOrEmpty(post.get("shangbanshijian"))) {
            return R.error("请填写上班时间");
        }
        // 判断是否有填写下班时间。
        if (StringUtil.isNullOrEmpty(post.get("xiabanshijian"))) {
            return R.error("请填写下班时间");
        }
     ...
     * @return 是否处理成功
     */
    public R<Object> update(Paiban entityData, Map post) {
        // 判断是否有填写上班时间。
        if (StringUtil.isNullOrEmpty(post.get("shangbanshijian"))) {
            return R.error("请填写上班时间");
        }
