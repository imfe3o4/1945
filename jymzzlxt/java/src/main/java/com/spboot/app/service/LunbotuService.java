...
public class LunbotuService {
     ...
     *  根据id 获取一行数据
     */
    public R<Lunbotu> findById(Integer id) {
     ...
     *  根据Wrapper 对象进行数据筛选
     */
    public R<List<Lunbotu>> selectAll(Wrapper<Lunbotu> query) {
    ...
    public R selectPages(QueryWrapper<Lunbotu> wrapper, IPage page) {
     ...
     * @return 是否处理成功
     */
    public R insert(Lunbotu entityData, Map post) {
        // 判断是否有填写图片。
     ...
     * @return 是否处理成功
     */
    public R<Object> update(Lunbotu entityData, Map post) {
        // 判断是否有填写图片。
        if (StringUtil.isNullOrEmpty(post.get("image"))) {
            return R.error("请填写图片");
        }
