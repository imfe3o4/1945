...
@Service
public class GerenjiankangshujuService {

    // 获取数据库操作类mapper
    @Resource
    private GerenjiankangshujuMapper mapper;

    /**
     *  根据id 获取一行数据
     */
    public R<Gerenjiankangshuju> findById(Integer id) {
     ...
     *  根据Wrapper 对象进行数据筛选
     */
    public R<List<Gerenjiankangshuju>> selectAll(Wrapper<Gerenjiankangshuju> query) {
    ...
    public R<List<Gerenjiankangshuju>> selectAll() {
        QueryWrapper<Gerenjiankangshuju> wrapper = Wrappers.query();
    ...
    public R selectAll(Map<String, Object> map) {
        ...
        QueryWrapper<Gerenjiankangshuju> wrapper = mapToWrapper(map);
        // 设置排序
    ...
    /**
     *  根据map 条件筛选yonghu字段等于session.username的数据并分页
     *
     */
    public R selectPagesYonghu(Map<String, Object> map) {
        // 获取前端给到列表基础参数
        SelectPage selectPage = new SelectPage(map, 10, "id", "DESC");
        // 将map参数转换为mybatis-plus的QueryWrapper类
        QueryWrapper<Gerenjiankangshuju> wrapper = mapToWrapper(map);
        // 设置为当前角色
        wrapper.eq("yonghu", SessionFactory.getUsername());
        wrapper.orderBy(true, selectPage.isAsc(), selectPage.getOrderby());
        Page page = new Page(selectPage.getPage(), selectPage.getPagesize());
        return selectPages(wrapper, page);
    }
     ...
     */
    public QueryWrapper<Gerenjiankangshuju> mapToWrapper(Map<String, Object> map) {
        // 创建 QueryWrapper 对象
        QueryWrapper<Gerenjiankangshuju> wrapper = Wrappers.query();
    ...
    public R selectPages(QueryWrapper<Gerenjiankangshuju> wrapper, IPage page) {
     ...
     * @return 是否处理成功
     */
    public R insert(Gerenjiankangshuju entityData, Map post) {
        // 判断是否有填写姓名。
        ...
        // 判断是否有填写日期。
        if (StringUtil.isNullOrEmpty(post.get("riqi"))) {
            return R.error("请填写日期");
        }
        // 判断是否有填写舒张压。
        if (StringUtil.isNullOrEmpty(post.get("shuzhangya"))) {
            return R.error("请填写舒张压");
        }
        // 判断是否有填写收缩压。
        if (StringUtil.isNullOrEmpty(post.get("shousuoya"))) {
            return R.error("请填写收缩压");
        }
        // 判断是否有填写血糖。
        if (StringUtil.isNullOrEmpty(post.get("xuetang"))) {
            return R.error("请填写血糖");
        }
        // 判断是否有填写心率。
        if (StringUtil.isNullOrEmpty(post.get("xinlv"))) {
            return R.error("请填写心率");
        }
     ...
     * @return 是否处理成功
     */
    public R<Object> update(Gerenjiankangshuju entityData, Map post) {
        // 判断是否有填写姓名。
        if (StringUtil.isNullOrEmpty(post.get("xingming"))) {
            return R.error("请填写姓名");
        }
