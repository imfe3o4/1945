...
public class ShijuanService {
    ...
    // 获取数据库操作类mapper
    @Resource
    private ShijuanMapper mapper;

    /**
     *  根据id 获取一行数据
     */
    public R<Shijuan> findById(Integer id) {
     ...
     *  根据Wrapper 对象进行数据筛选
     */
    public R<List<Shijuan>> selectAll(Wrapper<Shijuan> query) {
    ...
    public R<List<Shijuan>> selectAll() {
        QueryWrapper<Shijuan> wrapper = Wrappers.query();
    ...
    public R selectAll(Map<String, Object> map) {
        ...
        QueryWrapper<Shijuan> wrapper = mapToWrapper(map);
        // 设置排序
     ...
     */
    public QueryWrapper<Shijuan> mapToWrapper(Map<String, Object> map) {
        // 创建 QueryWrapper 对象
        QueryWrapper<Shijuan> wrapper = Wrappers.query();
    ...
    public R selectPages(QueryWrapper<Shijuan> wrapper, IPage page) {
     ...
     * @return 是否处理成功
     */
    public R insert(Shijuan entityData, Map post) {
        // 判断是否有填写试卷编号。
        if (StringUtil.isNullOrEmpty(post.get("shijuanbianhao"))) {
            return R.error("请填写试卷编号");
        }
        // 判断是否有填写试卷名称。
        if (StringUtil.isNullOrEmpty(post.get("shijuanmingcheng"))) {
            return R.error("请填写试卷名称");
        }
        // 判断是否有填写答题类型。
        if (StringUtil.isNullOrEmpty(post.get("datileixing"))) {
            return R.error("请填写答题类型");
        }
        // 判断是否有填写图片。
        if (StringUtil.isNullOrEmpty(post.get("tupian"))) {
            return R.error("请填写图片");
        }
        // 判断是否有填写时长。
        if (StringUtil.isNullOrEmpty(post.get("shichang"))) {
            return R.error("请填写时长");
        }
     ...
     * @return 是否处理成功
     */
    public R<Object> update(Shijuan entityData, Map post) {
        // 判断是否有填写试卷编号。
        if (StringUtil.isNullOrEmpty(post.get("shijuanbianhao"))) {
            return R.error("请填写试卷编号");
        }
