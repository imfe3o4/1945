...
import com.spboot.app.mapper.YaopinMapper;
...
public class YaopinService {
    ...
    // 获取数据库操作类mapper
    @Resource
    private YaopinMapper mapper;

    /**
     *  根据id 获取一行数据
     */
    public R<Yaopin> findById(Integer id) {
     ...
     *  根据Wrapper 对象进行数据筛选
     */
    public R<List<Yaopin>> selectAll(Wrapper<Yaopin> query) {
     ...
     *  直接筛选所有数据
     */
    public R<List<Yaopin>> selectAll() {
        QueryWrapper<Yaopin> wrapper = Wrappers.query();
    ...
    public R selectAll(Map<String, Object> map) {
        ...
        QueryWrapper<Yaopin> wrapper = mapToWrapper(map);
        // 设置排序
     ...
     */
    public QueryWrapper<Yaopin> mapToWrapper(Map<String, Object> map) {
        // 创建 QueryWrapper 对象
        QueryWrapper<Yaopin> wrapper = Wrappers.query();
        ...
        if (!StringUtil.isNullOrEmpty(map.get("keyword"))) {
            wrapper.apply(" ( mingcheng LIKE '%" + map.get("keyword") + "%' OR yaopinxiangqing LIKE '%" + map.get("keyword") + "%' ) ");
        }
    ...
    public R selectPages(QueryWrapper<Yaopin> wrapper, IPage page) {
        Map result = new HashMap();
        result.put("lists", mapper.selectPage(page, wrapper));

        wrapper.select("(sum(kucun)) sum_kucun,(sum(xiaoliang)) sum_xiaoliang");
     ...
     * @return 是否处理成功
     */
    public R insert(Yaopin entityData, Map post) {
        // 判断是否有填写编号。
        if (StringUtil.isNullOrEmpty(post.get("bianhao"))) {
        ...
        // 判断是否有填写价格。
        if (StringUtil.isNullOrEmpty(post.get("jiage"))) {
            return R.error("请填写价格");
        }
        // 判断是否有填写库存。
        if (StringUtil.isNullOrEmpty(post.get("kucun"))) {
            return R.error("请填写库存");
        }
     ...
     * @return 是否处理成功
     */
    public R<Object> update(Yaopin entityData, Map post) {
        // 判断是否有填写编号。
        if (StringUtil.isNullOrEmpty(post.get("bianhao"))) {
            return R.error("请填写编号");
        }
