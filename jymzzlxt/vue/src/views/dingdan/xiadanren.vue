                    ...
                    <el-table-column label="是否支付" width="120">
                        <template #default="{row}">
                            ...
                            <el-button
                                size="small"
                                type="primary"
                                @click="$goto({path:'/admin/pay' , query:{biao:'dingdan' , id:row.id,ordersn:row.dingdanhao,zongji:row.dingdanjine,isShaxiang:'' } })"
                                v-if="row.iszf=='否'"
                            >
                                去支付
                            </el-button>
...
<script setup>
    ...
    const loadList = (page) => {
        ...
        http.post("/api/dingdan/selectXiadanren", search).then(
            (res) => {
                loading.value = false;
                if (res.code == 0) {
                    var data = res.data;
