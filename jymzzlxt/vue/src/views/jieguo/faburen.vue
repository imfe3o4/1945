<template>
    <div class="views-jieguo-list">
        <div>
            <el-card class="box-card">
                <template #header>
                    <div class="clearfix">
                        <span class="title"> 结果查询 </span>
                    </div>
                </template>
                ...
                <div class="form-search">
                    <el-form @submit.prevent.stop :inline="true" size="small">
                        ...
                        <el-form-item label="答题类型">
                            <el-select v-model="search.datileixing"
                                ><el-option label="请选择" value=""></el-option><el-option label="每日答题" value="每日答题"></el-option>
                                <el-option label="健康自测" value="健康自测"></el-option>
                            </el-select>
                        </el-form-item>
                        <el-form-item label="答题人">
                            <el-input v-model="search.datiren"></el-input>
                        </el-form-item>
                        <el-form-item>
                ...
                <el-table border :data="lists" style="width: 100%" @sort-change="sortChange" highlight-current-row>
                    ...
                    <el-table-column prop="datileixing" label="答题类型" width="80">
                        <template #default="{row}"> {{ row.datileixing }} </template>
                    </el-table-column>
                    <el-table-column prop="kaoshibianhao" label="考试编号" width="130">
                        <template #default="{row}"> {{ row.kaoshibianhao }} </template>
                    </el-table-column>
                    ...
                    <el-table-column prop="daan" label="答案">
                        <template #default="{row}"> {{ row.daan }} </template>
                    </el-table-column>
                    <el-table-column prop="defen" label="得分" width="80">
                        <template #default="{row}"> {{ row.defen }} </template>
                    </el-table-column>
                    <el-table-column prop="zimu" label="字母" width="130">
                        <template #default="{row}"> {{ row.zimu }} </template>
                    </el-table-column>
                    <el-table-column prop="datiren" label="答题人" width="180">
                        <template #default="{row}"> {{ row.datiren }} </template>
                    </el-table-column>
                        ...
                        <template #default="{row}">
                            <el-button-group>
                                <el-tooltip effect="dark" content="详情" placement="top-start"
                                    ><el-button type="info" :icon="InfoFilled" size="small" @click="$router.push('/admin/jieguo/detail?id='+row.id)">详情</el-button>
                                </el-tooltip>
                                <el-tooltip effect="dark" content="编辑" placement="top-start"
                                    ><el-button type="success" :icon="Edit" size="small" @click="$router.push('/admin/jieguo/updt?id='+row.id)">批改</el-button>
                                </el-tooltip>
...
<script setup>
    ...
    import { canJieguoSelect, useJieguoSelect, canJieguoDelete } from "@/module";
    ...
    const loadList = (page) => {
        ...
        http.post("/api/jieguo/selectFaburen", search).then(
            (res) => {
                loading.value = false;
                if (res.code == 0) {
                    var data = res.data;
