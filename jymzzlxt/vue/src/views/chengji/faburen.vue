        ...
        <div>
            <el-card class="box-card">
                ...
                <el-table border :data="lists" style="width: 100%" @sort-change="sortChange" highlight-current-row>
                    ...
                    <el-table-column prop="zongdefen" label="总得分" width="80">
                        <template #default="{row}"> {{ row.zongdefen }} </template>
                    </el-table-column>
                        ...
                        <template #default="{row}">
                            <el-button-group>
                                <el-tooltip effect="dark" content="详情" placement="top-start"
                                    ><el-button type="info" :icon="InfoFilled" size="small" @click="$router.push('/admin/chengji/detail?id='+row.id)">详情</el-button>
                                </el-tooltip>
                                <el-tooltip effect="dark" content="编辑" placement="top-start"
                                    ><el-button type="success" :icon="Edit" size="small" @click="$router.push('/admin/chengji/updt?id='+row.id)">编辑</el-button>
                                </el-tooltip>
...
<script setup>
    ...
    import { canChengjiSelect, useChengjiSelect, canChengjiDelete } from "@/module";
    ...
    const loadList = (page) => {
        ...
        http.post("/api/chengji/selectFaburen", search).then(
            (res) => {
                loading.value = false;
                if (res.code == 0) {
                    var data = res.data;
