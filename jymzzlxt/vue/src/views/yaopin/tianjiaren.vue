<template>
    <div class="views-yaopin-list">
        <div>
            <el-card class="box-card">
                <template #header>
                    <div class="clearfix">
                        <span class="title"> 药品查询 </span>
                    </div>
                </template>
                ...
                <el-table border :data="lists" style="width: 100%" @sort-change="sortChange" highlight-current-row>
                    ...
                    <el-table-column sortable="custom" prop="kucun" label="库存" width="80">
                        <template #default="{row}"> {{ row.kucun }} </template>
                    </el-table-column>
                        ...
                        <template #default="{row}">
                            <el-button-group>
                                <el-tooltip effect="dark" content="详情" placement="top-start"
                                    ><el-button type="info" :icon="InfoFilled" size="small" @click="$router.push('/admin/yaopin/detail?id='+row.id)">详情</el-button>
                                </el-tooltip>
                                <el-tooltip effect="dark" content="编辑" placement="top-start"
                                    ><el-button type="success" :icon="Edit" size="small" @click="$router.push('/admin/yaopin/updt?id='+row.id)">编辑</el-button>
                                </el-tooltip>
                ...
                <p>库存总和: {{ total.sum_kucun }} 　　 销量总和: {{ total.sum_xiaoliang }} 　　</p>
            </el-card>
        </div>
    </div>
</template>
...
<script setup>
    ...
    const loadList = (page) => {
        ...
        http.post("/api/yaopin/selectTianjiaren", search).then(
            (res) => {
                loading.value = false;
                if (res.code == 0) {
                    var data = res.data;
