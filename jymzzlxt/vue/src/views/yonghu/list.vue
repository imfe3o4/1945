        ...
        <div>
            <el-card class="box-card">
                ...
                <el-table border :data="lists" style="width: 100%" @sort-change="sortChange" highlight-current-row>
                    ...
                    <el-table-column sortable="custom" prop="jifen" label="积分" width="80">
                        <template #default="{row}"> {{ row.jifen }} </template>
                    </el-table-column>
                    ...
                    <el-table-column prop="hunyinzhuangkuang" label="婚姻状况" width="80">
                        <template #default="{row}"> {{ row.hunyinzhuangkuang }} </template>
                    </el-table-column>
                    <el-table-column prop="minzu" label="民族" width="130">
                        <template #default="{row}"> {{ row.minzu }} </template>
                    </el-table-column>
                    <el-table-column prop="xianbingshi" label="现病史" width="130">
                        <template #default="{row}"> {{ row.xianbingshi }} </template>
                    </el-table-column>
                    <el-table-column prop="jiwangbingshi" label="既往病史" width="130">
                        <template #default="{row}"> {{ row.jiwangbingshi }} </template>
                    </el-table-column>
                    <el-table-column prop="jiazubingshi" label="家族病史" width="130">
                        <template #default="{row}"> {{ row.jiazubingshi }} </template>
                    </el-table-column>
                        ...
                        <template #default="{row}">
                            <el-button-group>
                                <el-tooltip effect="dark" content="编辑" placement="top-start"
                                    ><el-button type="success" :icon="Edit" size="small" @click="$router.push('/admin/yonghu/updt?id='+row.id)">编辑</el-button>
                                </el-tooltip>
...
<script setup>
    ...
    const exportExcel = () => {
        http.post("/api/yonghu/export", search)
            .json()
            .thenBlob(
                (data) => {
