<template>
    <div class="views-shijuan-list">
        <div>
            <el-card class="box-card">
                <template #header>
                    <div class="clearfix">
                        <span class="title"> 试卷查询 </span>
                    </div>
                </template>
                ...
                <el-table border :data="lists" style="width: 100%" @sort-change="sortChange" highlight-current-row>
                    ...
                    <el-table-column prop="shichang" label="时长" width="130">
                        <template #default="{row}"> {{ row.shichang }} </template>
                    </el-table-column>
                    <el-table-column prop="shijuanjianjie" label="试卷简介">
                        <template #default="{row}"> {{ row.shijuanjianjie }} </template>
                    </el-table-column>
                    ...
                    <el-table-column label="操作" fixed="right" width="180">
                        <template #default="{row}">
                            <el-button-group>
                                <el-button type="primary" :icon="Plus" size="small" @click="$router.push('/admin/shiti/add?id='+row.id)">添加试题</el-button>

                                <el-tooltip effect="dark" content="详情" placement="top-start"
                                    ><el-button type="info" :icon="InfoFilled" size="small" @click="$router.push('/admin/shijuan/detail?id='+row.id)">详情</el-button>
                                </el-tooltip>
                                <el-tooltip effect="dark" content="编辑" placement="top-start"
                                    ><el-button type="success" :icon="Edit" size="small" @click="$router.push('/admin/shijuan/updt?id='+row.id)">编辑</el-button>
                                </el-tooltip>
...
<script setup>
    ...
    const loadList = (page) => {
        ...
        http.post("/api/shijuan/selectFaburen", search).then(
            (res) => {
                loading.value = false;
                if (res.code == 0) {
                    var data = res.data;
