<template>
    <div class="views-xinlizhenliaofangan-list">
        <div>
            <el-card class="box-card">
                <template #header>
                    <div class="clearfix">
                        <span class="title"> 开具药方查询 </span>
                    </div>
                </template>
                ...
                <el-table border :data="lists" style="width: 100%" @sort-change="sortChange" highlight-current-row>
                    ...
                    <el-table-column prop="zhenliaofangan" label="诊疗方案">
                        <template #default="{row}"> {{ row.zhenliaofangan }} </template>
                    </el-table-column>
                    <el-table-column prop="chufang" label="处方">
                        <template #default="{row}"> {{ row.chufang }} </template>
                    </el-table-column>
                        ...
                        <template #default="{row}">
                            <el-button-group>
                                <el-tooltip effect="dark" content="详情" placement="top-start"
                                    ><el-button type="info" :icon="InfoFilled" size="small" @click="$router.push('/admin/xinlizhenliaofangan/detail?id='+row.id)">详情</el-button>
                                </el-tooltip>
                                <el-tooltip effect="dark" content="编辑" placement="top-start"
                                    ><el-button type="success" :icon="Edit" size="small" @click="$router.push('/admin/xinlizhenliaofangan/updt?id='+row.id)">编辑</el-button>
                                </el-tooltip>
...
<script setup>
    ...
    import { canXinlizhenliaofanganSelect, useXinlizhenliaofanganSelect, canXinlizhenliaofanganDelete } from "@/module";
    ...
    const exportExcel = () => {
        http.post("/api/xinlizhenliaofangan/export", search)
            .json()
            .thenBlob(
                (data) => {
    ...
    const deleteItems = (ids) => {
        return new Promise((resolve, reject) => {
            ElMessageBox.confirm("确定删除？")
                .then((res) => {
                    canXinlizhenliaofanganDelete(ids).then((res) => {
                        if (res.code == 0) {
    ...
    const loadList = (page) => {
        ...
        http.post("/api/xinlizhenliaofangan/selectZhanghao", search).then(
            (res) => {
                loading.value = false;
                if (res.code == 0) {
                    var data = res.data;
...
<style scoped lang="scss">
    .views-xinlizhenliaofangan-list {
    }
</style>
