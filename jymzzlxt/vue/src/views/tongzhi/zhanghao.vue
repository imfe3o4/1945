<template>
    <div class="views-tongzhi-list">
        <div>
            <el-card class="box-card">
                <template #header>
                    <div class="clearfix">
                        <span class="title"> 通知查询 </span>
                    </div>
                </template>
                ...
                <el-table border :data="lists" style="width: 100%" @sort-change="sortChange" highlight-current-row>
                    ...
                    <el-table-column prop="tongzhineirong" label="通知内容">
                        <template #default="{row}"> {{ row.tongzhineirong }} </template>
                    </el-table-column>
                        ...
                        <template #default="{row}">
                            <el-button-group>
                                <el-tooltip effect="dark" content="详情" placement="top-start"
                                    ><el-button type="info" :icon="InfoFilled" size="small" @click="$router.push('/admin/tongzhi/detail?id='+row.id)">详情</el-button>
                                </el-tooltip>
                                <el-tooltip effect="dark" content="编辑" placement="top-start"
                                    ><el-button type="success" :icon="Edit" size="small" @click="$router.push('/admin/tongzhi/updt?id='+row.id)">编辑</el-button>
                                </el-tooltip>
...
<script setup>
    ...
    import { canTongzhiSelect, useTongzhiSelect, canTongzhiDelete } from "@/module";
    ...
    const loadList = (page) => {
        ...
        http.post("/api/tongzhi/selectZhanghao", search).then(
            (res) => {
                loading.value = false;
                if (res.code == 0) {
                    var data = res.data;
