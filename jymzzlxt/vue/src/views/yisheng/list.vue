<template>
    <div class="views-yisheng-list">
        <div>
            <el-card class="box-card">
                <template #header>
                    <div class="clearfix">
                        <span class="title"> 医生查询 </span>
                    </div>
                </template>
                ...
                <el-table border :data="lists" style="width: 100%" @sort-change="sortChange" highlight-current-row>
                    ...
                    <el-table-column prop="zhaopian" label="照片" width="100">
                        <template #default="{row}"> <e-img :src="row.zhaopian" style="max-width: 120px" /> </template>
                    </el-table-column>

                    <el-table-column label="操作" fixed="right" width="350">
                        <template #default="{row}">
                            <el-button-group>
                                <el-button type="primary" :icon="Plus" size="small" @click="$router.push('/admin/paiban/add?id='+row.id)">添加排班</el-button>

                                <el-tooltip effect="dark" content="详情" placement="top-start"
                                    ><el-button type="info" :icon="InfoFilled" size="small" @click="$router.push('/admin/yisheng/detail?id='+row.id)">详情</el-button>
                                </el-tooltip>
                                <el-tooltip effect="dark" content="编辑" placement="top-start"
                                    ><el-button type="success" :icon="Edit" size="small" @click="$router.push('/admin/yisheng/updt?id='+row.id)">编辑</el-button>
                                </el-tooltip>
...
<script setup>
    ...
    import { canYishengSelect, useYishengSelect, canYishengDelete } from "@/module";
    ...
    const loadList = (page) => {
        ...
        http.post("/api/yisheng/selectPages", search).then(
            (res) => {
                loading.value = false;
                if (res.code == 0) {
                    var data = res.data;
